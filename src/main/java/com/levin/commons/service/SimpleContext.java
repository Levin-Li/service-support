...
@Tag(name = "上下文", description = "实现代码中要去区分null值和没有值;希望它能够代替一些线程变量;")
public interface SimpleContext<V> extends Supplier<V> {

    @Operation(summary = "是否有值", description = "如果放入null也算有值, 也就是说，只要设置了值，它就是有值")
    boolean hasValue();

    @Operation(summary = "设置值", description = "如果放入null也算有值")
    SimpleContext<V> set(V value);

    @Operation(summary = "清空一个值")
    SimpleContext<V> clear();
    ...
    default V computeIfAbsent(Supplier<V> supplier) {
        return hasValue() ? get() : set(supplier.get()).get();
    }

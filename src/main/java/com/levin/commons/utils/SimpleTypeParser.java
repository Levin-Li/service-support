...
//                    genricTypeMap.put(((Class) subTypeV.getGenericDeclaration()).getName() + "." + subTypeV.getName(), typeFromParent);
    ...
    public FieldInfo parse(FieldInfo parent, String name, final Type type, Type... actualTypeArguments) {
        ...
        if (type instanceof TypeVariable) {//如果发现是没有型别的泛型
            ...
            FieldInfo info = new FieldInfo(name, null, type, null);
            info.isList = true;
            ...
            FieldInfo info = new FieldInfo(name, null, type, (Class) type);
            info.isObject = true;
            ...
            for (Field f : typeClass.getDeclaredFields()) {
                ...
                if (!Modifier.isStatic(f.getModifiers())) {
                    ...
                    if (isPrimitive(genericType)
                        ...
                        FieldInfo e = new FieldInfo(f.getName(), f, genericType, f.getType());
                        e.isPrimitive = true;
            ...
            for (Method method : methods) {
                ...
                if (fieldNames.contains(fieldName) || fieldName.equals("class")) {
                    continue;
                }
                if (method.isAnnotationPresent(Desc.class)) {
                    final Type returnType = method.getReturnType();
                    FieldInfo e = new FieldInfo(fieldName, null, returnType, null);
                    e.isPrimitive = true;
                    e.annotations = new Annotation[]{method.getAnnotation(Desc.class)};
                    e.setParent(info);
                }
            }
    ...
    public static class FieldInfo implements Serializable {
        ...
        public FieldInfo(String name, Field field, Type valueType, Class<?> fieldType) {
            this.name = name;
            this.field = field;
        ...
        public String toString() {
            ...
            if (isShowDesc()) {
                ...
                Desc annotation = field != null
                        ? field.getAnnotation(Desc.class)
                        : (annotations != null && annotations.length > 0 && annotations[0] instanceof Desc ? (Desc) annotations[0] : null);

                if (annotation == null && fieldType != null && fieldType.isEnum()) {
                    annotation = fieldType.getAnnotation(Desc.class);
                }

                if (annotation != null) {

                    String desc = getInfo(annotation);

                    if (fieldType != null && fieldType.isEnum()) {

                        List<String[]> enumList = getEnumList(fieldType);

                        String enumInfo = "";

                        for (String[] item : enumList) {
                            enumInfo += item[1] + "(" + item[2] + "),";
                        }

                        desc += "  --> 枚举类型，可选值：[" + enumInfo + "]";
                    }

                    buf.append("    /* " + desc.replace("\r", "\\r").replace("\n", "\\n") + " */");
                }
...
//            if (field != null) {
//
//                List<Annotation> validationAnnos = findValidationAnnos(field.getAnnotations());
//
...
//            }
    ...
    public static String getInfo(Desc anno) {
        ...
        if (hasTxt(anno.code())) {
            hasV = true;
            sb.append("（").append(anno.code());
        }

        if (hasTxt(anno.detail())) {

            sb.append(hasV ? "," : "（").append(anno.detail());
    ...
    public static boolean hasTxt(String txt) {
        return txt != null && txt.trim().length() > 0;
    }


    public static List<Annotation> findValidationAnnos(Annotation... annotations) {

        List<Annotation> annotationList = new ArrayList<>(7);
    ...
    //------------------------------------------------------------------------------------------------------------------
    private static Map<Class/*EnumClass*/, Map<String/*name*/, String/*desc*/>> enumDescMap = new ConcurrentReferenceHashMap<>();

    public static String getDesc(Class enumClass, String enumName) {
    ...
    public static Map<String, String> getEnumMap(Class enumClass) {
        ...
        if (!enumDescMap.containsKey(enumClass)) {
            ...
            for (Field field : declaredFields) {
                ...
                if ("$VALUES".equals(field.getName()) || isIgnore(field)) {
                    continue;
                }
    ...
    public static List<String[/*0-值(索引值)，1-名称（enum的名称），2-描述*/]> getEnumList(Class enumClass) {
        ...
        if (enumClass != null) {
            ...
            for (Iterator iter = enums.iterator(); iter.hasNext(); ) {
                ...
                String enumDesc = getDesc(enumClass, e.name());
                String[] row = new String[]{i++ + "", e.name(), enumDesc != null ? enumDesc : e.name()};
                list.add(row);
            }
        }

...
@Documented
@Inherited
@Schema(title = "Each 循环功能渲染器。 文档：https://aisuda.bce.baidu.com/amis/zh-CN/components/each")
...
//////////////////////////////////////////////
   ...
   String value() default "	";
     ...
     *
     * 参考定义: {"type":"string","description":"用来控制通过什么字段读取成员数据，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "用来控制通过什么字段读取成员数据，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下")
    String itemKeyName() default "	";

    /**
     * 用来控制通过什么字段读取序号，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下
     *
     * 参考定义: {"type":"string","description":"用来控制通过什么字段读取序号，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "用来控制通过什么字段读取序号，考虑到可能多层嵌套 如果名字一样会读取不到上层变量，所以这里可以指定一下")
    String indexKeyName() default "	";

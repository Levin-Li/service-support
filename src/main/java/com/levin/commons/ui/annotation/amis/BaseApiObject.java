...
//////////////////////////////////////////////
   ...
   String value() default "	";
     ...
     * API 发送类型
     *
     * 参考定义: {"type":"string","enum":["get","post","put","delete","patch","jsonp","js"],"description":"API 发送类型"}
     *
     * 
     *
     * 
     ...
     * API 发送目标地址
     *
     * 参考定义: {"type":"string","description":"API 发送目标地址"}
     *
     * 
     *
     * 
     ...
     * 设置发送条件
     *
     * 参考定义: {"type":"string","description":"设置发送条件"}
     *
     * 
     *
     * 
     ...
     *
     * 参考定义: {"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"是否将两次返回的数据字段，做一个合并。配置返回对象中的字段名，支持配置多个。\n\n比如：同时返回 log 字段，第一次返回 {log: '1'}，第二次返回 {log: '2'}，合并后的结果是 {log: ['1', '2']]} 再比如：同时返回 items 字段，第一次返回 {items: [1, 2]}，第二次返回 {items: [3, 4]}，合并后的结果是 {items: [1, 2, 3, 4]}"}
     *
    ...
    @Schema(title = "是否将两次返回的数据字段，做一个合并。配置返回对象中的字段名，支持配置多个。\n\n比如：同时返回 log 字段，第一次返回 {log: '1'}，第二次返回 {log: '2'}，合并后的结果是 {log: ['1', '2']]} 再比如：同时返回 items 字段，第一次返回 {items: [1, 2]}，第二次返回 {items: [3, 4]}，合并后的结果是 {items: [1, 2, 3, 4]}")
    String concatDataFields() default "	";
     ...
     *
     * 参考定义: {"type":"string","description":"当开启自动刷新的时候，默认是 api 的 url 来自动跟踪变量变化的。 如果你希望监控 url 外的变量，请配置 trackExpression。"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "当开启自动刷新的时候，默认是 api 的 url 来自动跟踪变量变化的。 如果你希望监控 url 外的变量，请配置 trackExpression。")
     ...
     *
     * 参考定义: {"type":"boolean","description":"autoFill 是否显示自动填充错误提示"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "autoFill 是否显示自动填充错误提示")
    boolean silent() default false;

    /**
     * 提示信息
     *
     * 参考定义: {"type":"object","properties":{"success":{"type":"string"},"failed":{"type":"string"}},"additionalProperties":false,"description":"提示信息"}
     *
     * 
     *
     * 

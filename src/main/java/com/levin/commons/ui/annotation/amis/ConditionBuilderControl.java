...
@Documented
@Inherited
@Schema(title = "条件组合控件 文档：https://aisuda.bce.baidu.com/amis/zh-CN/components/form/condition-builder")
public @interface ConditionBuilderControl {
///////////////////////////////////////////
...
//////////////////////////////////////////////
     ...
     * 指定为
     *
     * 参考定义: {"type":"string","const":"condition-builder","description":"指定为"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "指定为")
    String type() default "condition-builder";

    /**
     * 内嵌模式，默认为 true
     *
     * 参考定义: {"type":"boolean","description":"内嵌模式，默认为 true"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "内嵌模式，默认为 true")
    boolean embed() default false;

    /**
     * 非内嵌模式时 弹窗触发icon
     *
     * 参考定义: "#/definitions/IconSchema"
     *
     * 
     *
     * 
     ...
     * 其他配置
     *
     * 参考定义: "#/definitions/ConditionBuilderConfig"
     *
     * 
     *
     * 
     *
     * @see ConditionBuilderConfig
     */
    
    @Schema(title = "其他配置")
    ConditionBuilderConfig config() ;

    /**
     * 通过远程拉取配置项
     *
     * 参考定义: {"anyOf":[{"$ref":"#/definitions/SchemaApi"},{"$ref":"#/definitions/SchemaTokenizeableString"}],"description":"通过远程拉取配置项"}
     *
    ...
    @Schema(title = "通过远程拉取配置项")
    String source() default "	";

    /**
     * 展现模式
     *
     * 参考定义: {"type":"string","enum":["simple","full"],"description":"展现模式"}
     *
     * 
     *
     * 
     ...
     *
     * 参考定义: {"type":"boolean","description":"是否显示并或切换键按钮，只在简单模式下有用"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "是否显示并或切换键按钮，只在简单模式下有用")
    boolean showANDOR() default false;

    /**
     * 是否可拖拽，默认为 true
     *
     * 参考定义: {"type":"boolean","description":"是否可拖拽，默认为 true"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "addBtnVisibleOn")
    String addBtnVisibleOn() default "	";

    /**
     * 表达式：控制按钮“添加条件组”的显示
     *
     * 参考定义: {"type":"string","description":"表达式：控制按钮“添加条件组”的显示"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "表达式：控制按钮“添加条件组”的显示")
    String addGroupBtnVisibleOn() default "	";

    /**
     * 将字段输入控件变成公式编辑器。
     *
     * 参考定义: {"type":"object","properties":{"evalMode":{"type":"boolean","description":"evalMode 即直接就是表达式，否则 需要 ${这里面才是表达式} 默认为 true"},"mixedMode":{"type":"boolean","description":"混合模式，意味着这个输入框既可以输入不同文本 也可以输入公式。 当输入公式时，值格式为 ${公式内容} 其他内容当字符串。"},"variables":{"type":"array","items":{"$ref":"#/definitions/VariableItem"},"description":"用于提示的变量集合，默认为空"},"variableMode":{"type":"string","enum":["tabs","tree"],"description":"变量展现模式，可选值：'tabs' ｜ 'tree'"},"functions":{"type":"array","items":{"$ref":"#/definitions/FuncGroup"},"description":"函数集合，默认不需要传，即  amis-formula 里面那个函数 如果有扩充，则需要传。"},"title":{"type":"string","description":"编辑器标题"},"header":{"type":"string","description":"顶部标题，默认为表达式"},"inputMode":{"type":"string","enum":["button","input-button","input-group"],"description":"控件模式"},"allowInput":{"type":"boolean","description":"外层input是否允许输入，否需要点击fx在弹窗中输入"},"icon":{"$ref":"#/definitions/SchemaIcon","description":"按钮图标"},"btnLabel":{"type":"string","description":"按钮Label，inputMode为button时生效"},"level":{"type":"string","enum":["info","success","warning","danger","link","primary","dark","light"],"description":"按钮样式"},"btnSize":{"type":"string","enum":["xs","sm","md","lg"],"description":"按钮大小"},"borderMode":{"type":"string","enum":["full","half","none"],"description":"边框模式，全边框，还是半边框，或者没边框。"},"placeholder":{"type":"string","description":"输入框占位符"},"variableClassName":{"type":"string","description":"变量面板CSS样式类名"},"functionClassName":{"type":"string","description":"函数面板CSS样式类名"},"selfVariableName":{"type":"string","description":"当前输入项字段 name: 用于避免循环绑定自身导致无限渲染"},"inputSettings":{"$ref":"#/definitions/FormulaPickerInputSettings","description":"输入框的类型"},"remark":{"$ref":"#/definitions/SchemaRemark","description":"显示一个小图标, 鼠标放上去的时候显示提示内容"},"labelRemark":{"$ref":"#/definitions/SchemaRemark","description":"显示一个小图标, 鼠标放上去的时候显示提示内容, 这个小图标跟 label 在一起"},"size":{"type":"string","enum":["xs","sm","md","lg","full"],"description":"表单项大小"},"label":{"anyOf":[{"type":"string"},{"type":"boolean","const":false}],"description":"描述标题"},"labelAlign":{"$ref":"#/definitions/LabelAlign","description":"描述标题"},"labelWidth":{"type":["number","string"],"description":"label自定义宽度，默认单位为px"},"labelClassName":{"type":"string","description":"配置 label className"},"name":{"type":"string","description":"字段名，表单提交时的 key，支持多层级，用.连接，如： a.b.c"},"extraName":{"type":"string","description":"额外的字段名，当为范围组件时可以用来将另外一个值打平出来"},"hint":{"type":"string","description":"输入提示，聚焦的时候显示"},"submitOnChange":{"type":"boolean","description":"当修改完的时候是否提交表单。"},"readOnly":{"type":"boolean","description":"是否只读"},"readOnlyOn":{"type":"string","description":"只读条件"},"validateOnChange":{"type":"boolean","description":"不设置时，当表单提交过后表单项每次修改都会触发重新验证， 如果设置了，则由此配置项来决定要不要每次修改都触发验证。"},"description":{"type":"string","description":"描述内容，支持 Html 片段。"},"desc":{"type":"string"},"descriptionClassName":{"$ref":"#/definitions/ClassName","description":"配置描述上的 className"},"mode":{"type":"string","enum":["normal","inline","horizontal"],"description":"配置当前表单项展示模式"},"horizontal":{"$ref":"#/definitions/FormHorizontal","description":"当配置为水平布局的时候，用来配置具体的左右分配。"},"inline":{"type":"boolean","description":"表单 control 是否为 inline 模式。"},"inputClassName":{"$ref":"#/definitions/ClassName","description":"配置 input className"},"required":{"type":"boolean","description":"是否为必填"},"validationErrors":{"type":"object","properties":{"isAlpha":{"type":"string"},"isAlphanumeric":{"type":"string"},"isEmail":{"type":"string"},"isFloat":{"type":"string"},"isInt":{"type":"string"},"isJson":{"type":"string"},"isLength":{"type":"string"},"isNumeric":{"type":"string"},"isRequired":{"type":"string"},"isUrl":{"type":"string"},"matchRegexp":{"type":"string"},"matchRegexp2":{"type":"string"},"matchRegexp3":{"type":"string"},"matchRegexp4":{"type":"string"},"matchRegexp5":{"type":"string"},"maxLength":{"type":"string"},"maximum":{"type":"string"},"minLength":{"type":"string"},"minimum":{"type":"string"},"isDateTimeSame":{"type":"string"},"isDateTimeBefore":{"type":"string"},"isDateTimeAfter":{"type":"string"},"isDateTimeSameOrBefore":{"type":"string"},"isDateTimeSameOrAfter":{"type":"string"},"isDateTimeBetween":{"type":"string"},"isTimeSame":{"type":"string"},"isTimeBefore":{"type":"string"},"isTimeAfter":{"type":"string"},"isTimeSameOrBefore":{"type":"string"},"isTimeSameOrAfter":{"type":"string"},"isTimeBetween":{"type":"string"}},"description":"验证失败的提示信息"},"validations":{"anyOf":[{"type":"string"},{"type":"object","properties":{"isAlpha":{"type":"boolean","description":"是否是字母"},"isAlphanumeric":{"type":"boolean","description":"是否为字母数字"},"isEmail":{"type":"boolean","description":"是否为邮箱地址"},"isFloat":{"type":"boolean","description":"是否为浮点型"},"isInt":{"type":"boolean","description":"是否为整型"},"isJson":{"type":"boolean","description":"是否为 json"},"isLength":{"type":"number","description":"长度等于指定值"},"isNumeric":{"type":"boolean","description":"是否为数字"},"isRequired":{"type":"boolean","description":"是否为必填"},"isUrl":{"type":"boolean","description":"是否为 URL 地址"},"matchRegexp":{"type":"string","description":"内容命中指定正则"},"matchRegexp1":{"type":"string","description":"内容命中指定正则"},"matchRegexp2":{"type":"string","description":"内容命中指定正则"},"matchRegexp3":{"type":"string","description":"内容命中指定正则"},"matchRegexp4":{"type":"string","description":"内容命中指定正则"},"matchRegexp5":{"type":"string","description":"内容命中指定正则"},"maxLength":{"type":"number","description":"最大长度为指定值"},"maximum":{"type":"number","description":"最大值为指定值"},"minLength":{"type":"number","description":"最小长度为指定值"},"minimum":{"type":"number","description":"最小值为指定值"},"isDateTimeSame":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"和目标日期相同，支持指定粒度，默认到毫秒"},"isDateTimeBefore":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"早于目标日期，支持指定粒度，默认到毫秒"},"isDateTimeAfter":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"晚于目标日期，支持指定粒度，默认到毫秒"},"isDateTimeSameOrBefore":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"早于目标日期或和目标日期相同，支持指定粒度，默认到毫秒"},"isDateTimeSameOrAfter":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"晚于目标日期或和目标日期相同，支持指定粒度，默认到毫秒"},"isDateTimeBetween":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"日期处于目标日期范围，支持指定粒度和区间的开闭形式，默认到毫秒, 左右开区间"},"isTimeSame":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"和目标时间相同，支持指定粒度，默认到毫秒"},"isTimeBefore":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"早于目标时间，支持指定粒度，默认到毫秒"},"isTimeAfter":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"晚于目标时间，支持指定粒度，默认到毫秒"},"isTimeSameOrBefore":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"早于目标时间或和目标时间相同，支持指定粒度，默认到毫秒"},"isTimeSameOrAfter":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"晚于目标时间或和目标时间相同，支持指定粒度，默认到毫秒"},"isTimeBetween":{"anyOf":[{"type":"string"},{"type":"array","items":{"type":"string"}}],"description":"时间处于目标时间范围，支持指定粒度和区间的开闭形式，默认到毫秒, 左右开区间"}}}]},"value":{"description":"默认值，切记只能是静态值，不支持取变量，跟数据关联是通过设置 name 属性来实现的。"},"clearValueOnHidden":{"type":"boolean","description":"表单项隐藏时，是否在当前 Form 中删除掉该表单项值。注意同名的未隐藏的表单项值也会删掉"},"validateApi":{"anyOf":[{"type":"string"},{"$ref":"#/definitions/BaseApiObject"}],"description":"远端校验表单项接口"},"autoFill":{"anyOf":[{"type":"object","additionalProperties":{"type":"string"}},{"type":"object","properties":{"showSuggestion":{"type":"boolean","description":"是否为参照录入模式，参照录入会展示候选值供用户选择，而不是直接填充。"},"api":{"anyOf":[{"$ref":"#/definitions/BaseApiObject"},{"type":"string"}],"description":"自动填充 api"},"silent":{"type":"boolean","description":"是否展示数据格式错误提示，默认为不展示","default":true},"fillMappinng":{"type":"object","description":"填充时的数据映射"},"trigger":{"type":"string","enum":["change","foucs"],"description":"触发条件，默认为 change"},"mode":{"type":"string","enum":["popOver","dialog","drawer"],"description":"弹窗方式，当为参照录入时用可以配置"},"position":{"type":"string","description":"当参照录入为抽屉时可以配置弹出位置"},"size":{"type":"string","description":"当为参照录入时可以配置弹出容器的大小"},"columns":{"type":"array","items":{},"description":"参照录入展示的项"},"filter":{"description":"参照录入时的过滤条件"}},"additionalProperties":false}],"description":"自动填充，当选项被选择的时候，将选项中的其他值同步设置到表单内。"},"initAutoFill":{"anyOf":[{"type":"boolean"},{"type":"string","const":"fillIfNotSet"}]},"$$id":{"type":"string","description":"组件唯一 id，主要用于页面设计器中定位 json 节点"},"className":{"$ref":"#/definitions/SchemaClassName","description":"容器 css 类名"},"$ref":{"type":"string","description":"配合 definitions 一起使用，可以实现无限循环的渲染器。"},"disabled":{"type":"boolean","description":"是否禁用"},"disabledOn":{"$ref":"#/definitions/SchemaExpression","description":"是否禁用表达式"},"hidden":{"type":"boolean","description":"是否隐藏"},"hiddenOn":{"$ref":"#/definitions/SchemaExpression","description":"是否隐藏表达式"},"visible":{"type":"boolean","description":"是否显示"},"visibleOn":{"$ref":"#/definitions/SchemaExpression","description":"是否显示表达式"},"id":{"type":"string","description":"组件唯一 id，主要用于日志采集"},"onEvent":{"type":"object","additionalProperties":{"type":"object","properties":{"weight":{"type":"number"},"actions":{"type":"array","items":{"$ref":"#/definitions/ListenerAction"}},"debounce":{"$ref":"#/definitions/debounceConfig"},"track":{"$ref":"#/definitions/trackConfig"}},"required":["actions"],"additionalProperties":false},"description":"事件动作配置"},"static":{"type":"boolean","description":"是否静态展示"},"staticOn":{"$ref":"#/definitions/SchemaExpression","description":"是否静态展示表达式"},"staticPlaceholder":{"type":"string","description":"静态展示空值占位"},"staticClassName":{"$ref":"#/definitions/SchemaClassName","description":"静态展示表单项类名"},"staticLabelClassName":{"$ref":"#/definitions/SchemaClassName","description":"静态展示表单项Label类名"},"staticInputClassName":{"$ref":"#/definitions/SchemaClassName","description":"静态展示表单项Value类名"},"staticSchema":{},"style":{"type":"object","description":"组件样式"},"editorSetting":{"type":"object","properties":{"behavior":{"type":"string","description":"组件行为、用途，如 create、update、remove"},"displayName":{"type":"string","description":"组件名称，通常是业务名称方便定位"},"mock":{"description":"编辑器假数据，方便展示"}},"description":"编辑器配置，运行时可以忽略"},"useMobileUI":{"type":"boolean","description":"可以组件级别用来关闭移动端样式"},"testIdBuilder":{"$ref":"#/definitions/TestIdBuilder"}},"required":["variables","functions","header"],"additionalProperties":false,"description":"将字段输入控件变成公式编辑器。"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "将字段输入控件变成公式编辑器。")
    String formula() default "	";

    /**
     * if 里面公式编辑器配置
     *
     * 参考定义: {"description":"if 里面公式编辑器配置"}
     *
     * 
     *
     * 
    ...
    @Schema(title = "if 里面公式编辑器配置")
    String formulaForIf() default "	";

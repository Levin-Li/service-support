...
@Documented
@Inherited
@Schema(description = "File 文件上传控件 文档：https://baidu.gitee.io/amis/docs/components/form/file")
...
//////////////////////////////////////////////
    ...
    @Schema(description = "如果上传的文件比较小可以设置此选项来简单的把文件 base64 的值给 form 一起提交，目前不支持多选。")
    ...
    @Schema(description = "如果不希望 File 组件上传，可以配置 `asBlob` 或者 `asBase64`，采用这种方式后，组件不再自己上传了，而是直接把文件数据作为表单项的值，文件内容会在 Form 表单提交的接口里面一起带上。")
    ...
    @Schema(description = "分块大小，默认为 5M.")
    ...
    @Schema(description = "1. 单选模式：当用户选中某个选项时，选项中的 value 将被作为该表单项的值提交， 否则，整个选项对象都会作为该表单项的值提交。 2. 多选模式：选中的多个选项的 `value` 会通过 `delimiter` 连接起来， 否则直接将以数组的形式提交值。")
